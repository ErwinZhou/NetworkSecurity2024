...
std::vector<uint8_t> DES::unpad(const std::vector<uint8_t> &data, size_t blockSize)
{
	...
	if (padLength > data.size() || padLength > blockSize)
		return data;
...
std::bitset<64> DES::execute(const std::bitset<64> &data, int mode)
{
	...
	std::cout << "Block in Hex before IP^-1: ";
	for (int i = 0; i < 64; i += 8)
	{
		std::cout << std::hex << ((block >> (56 - i)).to_ulong() & 0xFF) << " ";
	}
...
{
	std::bitset<48> block;
	// std::cout << "R in Hex before expansion: ";
	// for (int i = 0; i < 32; i += 8) // Change the iteration to be suitable for 32-bit
	// {
	// 	std::cout << std::hex << ((R >> (24 - i)).to_ulong() & 0xFF) << " "; // Adjust the shifting for 32-bit
	// }
	// std::cout << std::dec << std::endl;
	for (int i = 0; i < 48; i++)
	{
	...
	// std::cout << "Block in Hex after expansion: ";
	// for (int i = 0; i < 48; i += 8)
	// {
	// 	std::cout << std::hex << ((block >> (40 - i)).to_ulong() & 0xFF) << " ";
	// }
	// std::cout << std::dec << std::endl;
	block ^= K;

	std::cout << "Block in Hex after xor: ";
	for (int i = 0; i < 48; i += 8)
	{
		std::cout << std::hex << ((block >> (40 - i)).to_ulong() & 0xFF) << " ";
	}
	// std::cout << "Block in Bianary after xor: ";
	// std::cout << std::dec << std::endl;
	// for (int i = 47; i >= 0; i--)
	// {
	// 	std::cout << block[i];
	...
	for (int i = 0; i < 8; i++)
	{
		...
		std::cout << "BOX:" << static_cast<int>(sBoxVal) << std::endl;
		for (int k = 0; k < 4; k++)
		{
...
void DES::genSubKeys(const std::bitset<64> &key)
{
	...
	std::cout << "SubKeys in Hex:" << std::endl;
	for (int i = 0; i < 16; i++)
	{
		std::cout << "SubKey " << i + 1 << ": " << std::hex << subKeys[i].to_ullong() << std::endl;
	}
}
...
std::string DES::vecToHexStr(const std::vector<uint8_t> &vec)
{
	std::stringstream ss;
...
int main()
{
	DES des("133457799BBCDFF1"); // 使用合适的十六进制密钥字符串

	// 假设这是您的 char* 类型的明文
	char *plaintext_cstr = "12345678"; // 确保这个字符串长度符合您的需求

	// 将 char* 转换为 std::vector<uint8_t>
	std::vector<uint8_t> plaintext(plaintext_cstr, plaintext_cstr + std::strlen(plaintext_cstr));

	// 加密
	std::vector<uint8_t> ciphertext = des.encrypt(plaintext);

	// 解密
	std::vector<uint8_t> decText = des.decrypt(ciphertext);

	// 打印结果
	std::cout << "Plaintext (Hex): " << des.vecToHexStr(plaintext) << std::endl;
	std::cout << "Ciphertext (Hex): " << des.vecToHexStr(ciphertext) << std::endl;
	std::cout << "Decrypted Text (Hex): " << des.vecToHexStr(decText) << std::endl;
}
